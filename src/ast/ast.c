...
static size_t node_size(AstNodeType type) {
    ...
    (type == AST_EXPR_OR)? sizeof(ast_expr_or_t) : 
    (type == AST_EXPR_AND)? sizeof(ast_expr_and_t) : 
    (type == AST_EXPR_EQUALITY)? sizeof(ast_expr_equality_t) : 
    (type == AST_EXPR_COMPARE)? sizeof(ast_expr_compare_t) : 
    (type == AST_EXPR_TERM)? sizeof(ast_expr_term_t) : 
    (type == AST_EXPR_FACTOR)? sizeof(ast_expr_factor_t) : 
    (type == AST_EXPR_UNARY)? sizeof(ast_expr_unary_t) : 
